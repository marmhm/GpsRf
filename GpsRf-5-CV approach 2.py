...
import datetime
interact= np.loadtxt("drug_drug_matrix.txt",dtype=int)
drug_drug_matrix=np.loadtxt("drug_drug_matrix.txt",dtype=int)
...
sim=np.loadtxt("snf_normal.txt",dtype=float)
    ...
    for i in range(0, len(drug_drug_matrix)):
        for j in range(i + 1, len(drug_drug_matrix)):
            if drug_drug_matrix[i, j] == 1:
                ...
                link_number_0 = link_number_0 + 1
     
    link_position = np.array(link_position)
    nonLinksPosition = np.array(nonLinksPosition)
    link_number_01=list(np.arange(0, link_number_0))
     


    index = np.arange(0, link_number)

    index0n = random.sample(link_number_01, link_number)
 
    print("0n",len(index0n))
    index0n=np.asarray(index0n)
    random.seed(seed)
    random.shuffle(index)
    # random.shuffle(index0n)



    fold_num = link_number // CV_num
    print("foldnum",fold_num)

    for CV in range(0,5):
        print('*********round:' + str(CV) + "**********\n")
        test_index = index[(CV * fold_num):((CV + 1) * fold_num)]
        test_index.sort()
        testLinkPosition_1 = link_position[test_index]
		
        test_index_0= index0n[(CV * fold_num):((CV + 1) * fold_num)]
        test_index_0.sort()
        testLinkPosition_0 = nonLinksPosition[test_index_0]
		
		
        testLinkPosition=list(testLinkPosition_1)+list(testLinkPosition_0) 
        
        testLinkPosition=np.asarray(testLinkPosition)
        train_drug_drug_matrix = copy.deepcopy(drug_drug_matrix)

        
        train_indx = (set(index[:]) - set(test_index) )
        train_indx=np.asarray(list(train_indx))
        train_indx.sort()
        trainLinkPosition = link_position[train_indx]

        train_indx_0 = np.asarray(list(set(index0n[:]) - set(test_index_0)))
        train_indx_0.sort()
        trainLinkPosition_0 = nonLinksPosition[train_indx_0]
		
        trainposition=list( trainLinkPosition)+list(trainLinkPosition_0)
        print("lentrain",len(trainposition))
        ...
        for (a, b) in trainposition:
            X_train.append(a * col + b)
...
# calculate score2
        ...
        path3=(np.matmul(np.matmul(act,sim2),act))+(np.matmul(np.matmul(sim2,sim2),act))+\
              (np.matmul(np.matmul(act,sim2),sim2))+(np.matmul(np.matmul(sim2,act),sim2))+(np.matmul(np.matmul(act,act),act))
        s2=path2+path3
